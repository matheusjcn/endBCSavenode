...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (!(await schema.isValid(req.body))) {
      return res.status(400).json({ error: 'Validation appointments fails' });
    }
    ...
    // Check User and Provider availability
    if (provider_id === req.userId) {
      return res
        .status(401)
        .json({ error: 'You dont create appointment with yourself' });
    }
